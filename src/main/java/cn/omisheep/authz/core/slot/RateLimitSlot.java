...
public class RateLimitSlot implements Slot {
    ...
    public void chain(HttpMeta httpMeta, HandlerMethod handler, Error error) {
        ...
        Object    userId    = null;
        if (httpMeta.isHasToken()) userId = httpMeta.getToken().getUserId();
        ...
        if (requestMeta != null && requestMeta.isBan()) {
            if (!requestMeta.enableRelive(now)) {
                httpMeta.log(LogLevel.WARN, "「请求频繁、{}封锁(拒绝)」 \t api: [{}] , 距上次访问: [{}] , method: [{}] , ip : [{}] , userId : [{}] , path: [{}]  ", checkType, api, requestMeta.sinceLastTime(), method, ip, userId, path);
                error.error(ExceptionStatus.REQUEST_REPEAT);
                requestMeta.setLastRequestTime(now);
                return;
            } else {
                httpMeta.log("「解除{}封禁(解封)」 \t api: [{}] , 距上次访问: [{}] , method: [{}] , ip : [{}] , userId : [{}] , path: [{}]  ", checkType, api, requestMeta.sinceLastTime(), method, ip, userId, path);
        ...
        if (requestMeta == null) {
            ...
            if (requestMeta.request(now, limitMeta.getMaxRequests(), limitMeta.getWindow(), limitMeta.getMinInterval())) {
                ...
                httpMeta.log(LogLevel.WARN, "「请求频繁、{}封锁(封禁)」 \t api: [{}] , 距上次访问: [{}] , method: [{}] , ip : [{}] , userId : [{}] , path: [{}]  ", checkType, api, requestMeta.sinceLastTime(), method, ip, userId, path);
                error.error(ExceptionStatus.REQUEST_REPEAT);
            }
        }
